...
xText .append("text")
    .attr("x", 5)
    .attr("data-name", "avg_Wind_Chill")
    .attr("data-axis", "x")
    .attr("class", "aText inactive x")
    .text("Average Wind Chill");
	
xText .append("text")
    .attr("x", 25)
    .attr("data-name", "avg_Wind_Mph")
    .attr("data-axis", "x")
    .attr("class", "aText inactive x")
    .text("Average Wind Chill");
...
d3.csv("assets/data/game_info.csv").then(function(data) {
    visualize(data);
});
...
function visualize (csvData) {
   ...
   var toolTip = d3.tip()
      ...
      .html(function(d) {
            //Build text box
            var stateLine = `<div>${d.season}</div>`;
